...
contract PROPHET is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        // uint256 amountPercent = (amount * 100) / _totalSupply;
        // require(_transferCount > 30 || amountPercent <= 5, "You Cannot transfer this much amount until 30 buys.");
        ...
        // if account is excludeFromFee account then don't collect taxFee and burnFee
        if(_isExcludedFromFee[to])
            removeAllFee();
        
        //transfer all tokens to this address (we will return the remaining tokens after taking the liquidityfee)
        _tokenTransfer(from, address(this), amount);

        
        //casue we have already taken fee in the above step
        removeAllFee();

        uint256 ammountToTransfer = balanceOf(address(this));
        if(!_isExcludedFromFee[to]){
            //take liquidityfee
            uint256 liquidityfee = amount.mul(_liquidityFee).div(100);
            ammountToTransfer = ammountToTransfer.sub(liquidityfee);
        }
        
        //transfer tokens and collect taxFee and burnFee
        _tokenTransfer(address(this), to, ammountToTransfer);
        
        //add the address to token holders list, to track all token holders (used when distributing rewards)
        ...
        //just to add all fee if was removed previously for the excludeFromFee account
        addAllFee();
    }
    ...
    function withDrawLpTokens(address _token) public onlyOwner {
