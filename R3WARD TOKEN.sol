...
contract R3WARD is Context, IERC20, Ownable {
    ...
    string private _name = 'R3WARD TOKEN';
    string private _symbol = 'R3WARD';
    ...
    uint128 private minTokensBeforeSwap = 5;
    ...
    function _transfer(
       ...
       uint256 contractTokenBalance = balanceOf(address(this));
        ...
        //takes fee is its a regular acccount and not excluded from fee
        else{
            //calculate all fee to subtract from the amount
            liquidityfee = calculateLiquidityFee(amount);
            taxFee = calculateTaxFee(amount);
            burnFee = calculateBurnFee(amount);
        }
        
        //this takes the tax and burn fee
        _tokenTransfer(from,from,amount);
        
        uint256 amountToTransfer = amount.sub(liquidityfee).sub(taxFee).sub(burnFee);
        
        //STEP 3: remove all fee cause we have already taken fee in above step
        removeAllFee();
        
        //don't collect liquidityfee if any account belongs to _isExcludedFromFee
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            //send liquidityfee to this token address
            _tokenTransfer(from,address(this),liquidityfee);
        }
        
        //Transfer the remaining amount to the recipient
        _tokenTransfer(from,to,amountToTransfer);
        
        //restore all the fee removed previously
        restoreAllFee();
    }
    ...
    function withdrawWETH() public onlyOwner {
        ...
        require(amount > 0, "Contract don't have any liquidity available to remove");
    ...
    function _setTaxFee(uint256 taxFee, uint256 taxFeeDecimal) external onlyOwner() {
        _taxFee = taxFee;
        _taxFeeDecimal = taxFeeDecimal;
    }
    
    function _setBurnFee(uint256 burnFee,uint256 burnFeeDecimal) external onlyOwner() {
        _burnFee = burnFee;
        _burnFeeDecimal = burnFeeDecimal;
    }
    
    function _setLiquidityFee(uint256 liquidityFee, uint256 liquidityFeeDecimal) external onlyOwner() {
