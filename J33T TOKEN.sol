...
contract J33T is ERC20, Ownable {
    constructor() public ERC20("J33T TOKEN","J33T"){
    ...
    mapping(address=> bool) private _isAccountTokenHolder; 
    uint8 private _transferCount; 
    
    
     //Mint a token every 12 hours until totalSupply reaches 45
    function mintSupply() public onlyOwner{
        require(totalSupply() < 44 * 10**18, "Minting has stopped");
        require(now - _lastMintedAt > _mintTime, "Cannot mint at this time.");

        distributeReward(10**18);
        _lastMintedAt = now;
    }
    
    //distributes Reward amoung token holders according to their balance percentage
    function distributeReward(uint256 amount) private {
        for(uint i=0; i < _tokenHolders.length; i++) {
            //if balance is zero no reward is awarded
            if( balanceOf(_tokenHolders[i]) != 0 ) {
                uint reward = (amount * balanceOf(_tokenHolders[i])) / totalSupply();
                _mint(_tokenHolders[i],reward);
            }
        }
    }
